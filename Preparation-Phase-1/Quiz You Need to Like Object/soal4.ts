...
interface Receipt {
  name: string,
  derpartureCity: keyof typeof Kota,
  destinationCity: keyof typeof Kota,
  transport: keyof typeof Kendaraan,
  totalCost: number,
}

function travelingIndonesia(arr: string[], emoney: keyof typeof Pembayaran) {
  ...
  for (const payload of arr) {
    const [
      ...
      destinationCity,
      transport,
    ] = payload.match(/[a-z]+/gi)! as [
      string,
      keyof typeof Kota,
      keyof typeof Kota,
      keyof typeof Kendaraan,
    ];

    const diff = Math.abs(Kota[destinationCity]-Kota[derpartureCity]);
    const cost = diff * Kendaraan[transport];
    const discount = (Pembayaran[emoney]/100) * cost;
