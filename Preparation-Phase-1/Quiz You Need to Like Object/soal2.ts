...
function economyChangeSummary(tradeActivity: string[][]) {
  let duitJeff = 100000
  let duitLarry = 95000
  let duitJack = 90000

  const summary: TradeReceipt[] = [];

  for (const list of tradeActivity) {
    for (const trade of list) {
      const [name, operation, percent] = trade.match(/[a-z 0-9]+|[-+]/gi)!;
      const doCalc = (duit: number) => ((parseInt(percent)/100) * duit) * (operation === "+" ? 1 : -1);
      const pushSummary = (deposit: number, owner: string) => summary.push({ name, deposit, owner});
      switch(name) {
        case "Jeff Bezos":
          duitJeff += doCalc(duitJeff);
          pushSummary(duitJeff, "Amazon");
          break;
        case "Larry Page":
          duitLarry += doCalc(duitLarry);
          pushSummary(duitLarry, "Google");
          break;
        case "Jack Ma":
          duitJack += doCalc(duitJack);
          pushSummary(duitJack, "Alibaba");
          break;
        default:
          break;
      }
