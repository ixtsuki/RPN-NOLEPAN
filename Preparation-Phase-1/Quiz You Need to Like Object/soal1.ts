...
function ganjilGenapAsianGames(date: number, data: DataKendaraan[]) {
  if (date < 1 || date > 31) throw RangeError("Invalid Date must be between 1 - 31");
  const tipeTanggal = date % 2;
  let result = 0;
  for (const kendaraan of data) 
    if (kendaraan.type === "Mobil" && parseInt(kendaraan.plat.split(" ")[1]) % 2 !== tipeTanggal)
      result++;
  return result;
}
