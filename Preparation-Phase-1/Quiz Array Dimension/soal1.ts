...
function ini akan memutar elemen array,
string yang berada di dalam elemen array juga akan dibalik atau diputar jika panjang string GENAP
...
function doubleReverse(arr: string[] | string) {
  const isArray = arr instanceof Array;
  if (isArray && arr.length < 1) throw new RangeError("Invalid input parameter");
  const newArr: string[] = [];
  
  for (let i = arr.length - 1; i >= 0; i--) {
    let str = arr[i];
    if (isArray && str.length % 2 === 0) {
      str = doubleReverse(str).join("");
    }
    newArr.push(str);
  }
  return newArr;
