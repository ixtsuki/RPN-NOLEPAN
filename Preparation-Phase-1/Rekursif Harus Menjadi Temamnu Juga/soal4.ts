...
function dividableRecursive(array: number[], num: number) {
  switch(array.length) {
    case 1:
      return array[0] % num === 0 ? [array[0]] : []
    default:
      return dividableRecursive([array[0]], num).concat(dividableRecursive(array.slice(1), num));
  }
}
