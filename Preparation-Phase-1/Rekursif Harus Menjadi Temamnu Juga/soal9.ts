...
function virusCheckRecursive(str: string, viruses?: string) {
  if (!viruses) return "there is no virus";
  str = str.toLowerCase();
  switch(str.length) {
    case 1:
      for (const virus of viruses.toLowerCase().split("|"))
        if (str === virus) return 1;
      return 0;
    default:
      return virusCheckRecursive(str[0], viruses) + virusCheckRecursive(str.slice(1), viruses);
  }
}
