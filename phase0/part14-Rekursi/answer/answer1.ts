...
function sorting(arrNumber: number[]) {
  ...
  function getTotal(arrNumber: number[]) {
    ...
    const highest = arrNumber[0];
    const munculCount = arrNumber.filter(x => x === highest).length;
    return `angka paling besar adalah ${highest} dan jumlah kemunculan sebanyak ${munculCount} kali`;
  }
