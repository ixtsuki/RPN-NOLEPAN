...
type GraduatesReturn = {[key: string]: { name: string, score: number }[]};

function graduates (students: NilaiSiswa[]): GraduatesReturn {
    const result = {} as GraduatesReturn;
    for (const student of students) {
        if (!result[student.class]) result[student.class] = [];
        if (student.score > 75) result[student.class].push({
            name: student.name,
            score: student.score,
        })
    }
